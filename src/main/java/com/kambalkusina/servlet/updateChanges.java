...
public class updateChanges extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String payment1 = request.getParameter("paymentChange");
		...
		PreparedStatement pst = con.prepareStatement("update customerorders SET payment = ? WHERE uname = ? AND product = ? AND datentime = ?");
		pst.setString(1, payment1);
		pst.setString(2, uname);
		pst.setString(3, product);
		pst.setString(4, datentime);
